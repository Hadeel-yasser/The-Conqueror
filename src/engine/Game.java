...
public class Game {
	...
	private final int maxTurnCount = 50; // change to 50?
	...
	public void targetCity(Army army, String targetName) {
		...
		for (Distance d : distances) {
			if ((d.getFrom().equals(from) || d.getFrom().equals(targetName))
				...
				if (army.getCurrentLocation().equals("onRoad"))
					distance += army.getDistancetoTarget();// reach 1 city first--> distance then add the new target city distance
	...
	public void endTurn() {
		...
		for (Army a : player.getControlledArmies()) {
			...
			totalUpkeep +=  a.foodNeeded(); // will count defending army's food again?
